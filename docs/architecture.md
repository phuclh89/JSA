...
JSA permission-code mapping remains a deployment configuration boundary. All four capabilities must be supplied together and `SYSTEM_ADMIN` is never an implicit JSA capability. Draft creation uses the configured template/sequence only for a Temporary JSA Number. Initial publication allocates the Official JSA Number from the locked `JSA_NUMBER_COUNTER` row for the exact Rig/Department pair, formats `<Rig name>-<Department code>-NNNN` from governed organization data, updates the Master number status, and publishes in the same transaction. The counter remains keyed by immutable identifiers rather than display text, and the database prevents later mutation of an Official number.
