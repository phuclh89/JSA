...
- JSAMS uses enterprise identity-provider authentication. Never implement or store local user passwords unless a future explicitly approved architecture decision supersedes this rule.
...
| `docs/decisions/ADR-005-ldap-enterprise-authentication.md`     | Accepted decision for internal Active Directory authentication through LDAP, stable identity mapping, and signed JSAMS sessions.                                                                                | A task touches enterprise login, LDAP configuration, session handling, password exposure boundaries, or proposes another authentication protocol.                                                     | Its status or supersession metadata changes. Use a new ADR for a replacement decision.                                                                                                                                 |
| `docs/decisions/ADR-006-ldap-direct-bind-strategy.md`          | Accepted decision making Direct Bind the primary runtime LDAP strategy while retaining optional service-account search.                                                                                         | A task touches LDAP bind strategy, accepted domain forms, directory credential validation, or service-account requirements.                                                                           | Its status or supersession metadata changes. Use a new ADR for a replacement decision.                                                                                                                                 |
| `docs/decisions/ADR-007-site-filesystem-attachment-library.md` | Accepted decision for site filesystem binary storage, immutable attachment versions, third-party file synchronization, and GoldenGate metadata-only replication.                                                | A task touches attachment storage, upload/download, file versioning, mapped drives, cross-site binary synchronization, or attachment metadata replication.                                            | Its status or supersession metadata changes. Use a new ADR for a replacement decision.                                                                                                                                 |
