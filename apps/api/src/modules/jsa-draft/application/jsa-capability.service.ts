...
export class JsaCapabilityService {
  ...
  capabilities(user: AuthenticatedUser): JsaDraftCapabilities {
    ...
    const has = (cap: JsaDraftCapability) =>
      Boolean(codes[cap] && user.permissions.includes(codes[cap]!));
    return {
      view: has('view'),
      create: has('create'),
      edit: has('edit'),
      ...
      configured,
      ...(!configured
        ? { unavailableReason: 'JSA permission-code mapping is not configured' }
        : {}),
    };
  }
  ...
  private codes(): Record<JsaDraftCapability, string | undefined> {
    return {
      view: this.config.get<string>('JSA_PERMISSION_VIEW'),
      create: this.config.get<string>('JSA_PERMISSION_CREATE'),
      edit: this.config.get<string>('JSA_PERMISSION_EDIT'),
      cancel: this.config.get<string>('JSA_PERMISSION_CANCEL'),
    };
  }
}
