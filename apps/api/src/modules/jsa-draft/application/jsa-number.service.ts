...
export class JsaNumberService {
  ...
  async next(
    ...
    siteId: string,
  ): Promise<{ number: string; scopeKey: string }> {
    ...
    if (!template || !template.includes('{sequence}') || !['GLOBAL', 'SITE'].includes(scope ?? ''))
      throw new StateConflictError('Temporary JSA numbering configuration is unavailable');
    const result = await context.connection.execute<{ VALUE: string }>(
      `SELECT TO_CHAR(SEQ_JSA_BUSINESS_NUMBER.NEXTVAL) VALUE FROM DUAL`,
      {},
      { outFormat: oracledb.OUT_FORMAT_OBJECT },
    );
    const value = result.rows?.[0]?.VALUE;
    if (!value) throw new StateConflictError('Temporary JSA number could not be generated');
    const generated = template.replaceAll('{sequence}', value).replaceAll('{siteId}', siteId);
    if (generated.length > 100)
      throw new StateConflictError('Configured temporary JSA number exceeds 100 characters');
