...
`pnpm db:seed:legacy-hazard-prompts` idempotently creates or aligns the confirmed 25 Hazard Assessment Prompts at `GLOBAL` scope, using `HAZARD_PROMPT_SEED_ACTOR` for the audit actor. Existing governed Prompt IDs are retained when prior Rig-scoped rows are promoted to Global; duplicate active copies are soft-deactivated. The seed also soft-deactivates the three superseded development fixtures (`ENERGY`, `DROPPED`, and `PINCH`). Run `pnpm db:verify:legacy-hazard-prompts` to check the exact labels, display order, Global scope, and shared 25-item count.

`pnpm db:seed:pv-drilling-positions` idempotently creates or aligns the confirmed 35 Position names at `GLOBAL` scope, using `POSITION_SEED_ACTOR` for the audit actor. An existing active row matched by stable code or exact name is promoted in place so its Position ID remains available to historical references; duplicate active scoped copies are soft-deactivated. Run `pnpm db:verify:pv-drilling-positions` to verify the exact code/name/order and null Site/Rig/Department ownership.
...
Migration 011 separates Temporary Draft/approval identifiers from Official JSA numbers. It adds `JSA_MASTER.NUMBER_STATUS`, creates the concurrency-safe `JSA_NUMBER_COUNTER` keyed by Rig/Department, constrains the counter to `0001`–`9999`, and installs `TRG_JSA_OFFICIAL_NUM_IMMUTABLE`. Final approval locks and increments the exact pair counter and publishes `<Rig name>-<Department code>-NNNN` in the same transaction. The display segment uses the governed Rig Name while counter identity remains the Rig/Department IDs. Rollback removes only the counter, trigger, and number-status column; it does not rewrite `JSA_NUMBER`.
