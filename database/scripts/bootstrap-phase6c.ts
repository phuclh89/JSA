...
async function main() {
  ...
  try {
    ...
    if (!approved?.RANGE_START || !approved.RANGE_END || approved.VARIANTS !== 1)
      throw new Error('Phase 6C bootstrap refused: Site range is missing or inconsistent');
    ...
    if (existing.rows?.[0]?.C !== 0)
      throw new Error('Phase 6C bootstrap refused: inconsistent sequence configuration');
